...
const handler = async (m, {conn, text, usedPrefix, command}) => {
  ...
  if (!text) throw `*[❗] Pergunte algo para usar a função do ChatGPT*\n\n*—◉ Exemplo:*\n*◉ ${usedPrefix + command} Reflexão sobre a série Breaking Bad de netflix*\n*◉ ${usedPrefix + command} Código em JS para um jogo de cartas*`;
  try {
        conn.sendPresenceUpdate('composing', m.chat);
        //let sistema1 = await fetch(`https://raw.githubusercontent.com/Skidy89/chat-gpt-jailbreak/main/Text.txt`).then(v => v.text());
        let sistema1 = `Atuaras como um Bot de WhatsApp no qual foi criado por SpyHacker, tu serás The Spy - Bot.`;
      ...
      try {
        conn.sendPresenceUpdate('composing', m.chat);
        const syms1 = `Atuaras como um Bot de WhatsApp no qual foi criado por SpyHacker, tu serás The Spy - Bot.`;
...
handler.command = /^(openai|chatgpt|ia|robot|openai2|chatgpt2|ia2|gpt|thespy|spybot)$/i;
