...
handler.all = async function(m) {
  ...
  if (/^big$/i.test(m.text) && !chat.isBanned) {
    conn.sendPresenceUpdate('recording', m.chat);
    await m.reply(`*Big MC* meu mano criador do grupo, já que ta falado dele, da uma moral e olha a porra da descrição do grupo pra se inscrever no canal dele, é isso, flw`);
