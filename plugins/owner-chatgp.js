...
const handler = async (m, {conn, text, groupMetadata}) => {
  ...
  if (!text) throw '*_⚠ • ️Escreva um -texto- para enviar uma mensagem a todos os grupos._*';
  const linkThisGroup = `${link}`;
  if (m.text.includes(linkThisGroup)) return conn.reply(m.chat, '❌ *_Não podes espamar links para outros grupos._*', m);
  const time = global.db.data.users[m.sender].msgwait + 300000;
  if (new Date - db.data.users[m.sender].msgwait < 300000) throw `*_⚠️ • Tens que esperar ${msToTime(time - new Date())} para voltar a enviar mensagens._*`;
