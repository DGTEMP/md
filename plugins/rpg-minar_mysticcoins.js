const handler = async (m, {conn, isPrems}) => { // lastmiming
  ...
  const minar = `${pickRandom(['Que pro 😎 já minerado',
    '🌟✨ Genial!! Conseguisse',
    'WOW!! és um(a) grande Minerador(a) ⛏️ Obtivesse',
    'Minerou!!',
    '😲 Você conseguiu minerar a quantidade de',
    'Sua renda vai aumentar graças à sua mineração',
    '⛏️⛏️⛏️⛏️⛏️ Minando',
    '🤩 SIIIIII!!! AGORA TENS',
    'Os minérios estão do teu lado, por isso Conseguisse',
    '😻 A sorte de Minar',
    '♻️ Sua missão foi cumprida, você conseguiu minerar',
    '⛏️ A mineração beneficiou você com',
    '🛣️ Você encontrou um lugar e, ao minerar esse lugar, conseguisse',
    '👾 Você consegue, graças ao fato de ter minerado sua renda somada',
    'Felicidades!! Agora tens', '⛏️⛏️⛏️ Conseguisse'])}`;
  ...
  if (new Date - user.lastcoins < 600000) return await conn.reply(m.chat, `*⏱️ 𝙑𝙤𝙩𝙩𝙚 𝙚𝙢 ${msToTime(time - new Date())} 𝙥𝙖𝙧𝙖 𝙘𝙤𝙣𝙩𝙞𝙣𝙪𝙖𝙧 𝙢𝙞𝙣𝙚𝙧𝙖𝙣𝙙𝙤 ${global.rpgshopp.emoticon('money')}⛏️*`, m);
