...
const handler = async (m, {conn, command, args, text, usedPrefix}) => {
  if (!text) throw `_*< SPY - PLAY />*_\n\n*[ ℹ️ ] Falta o título ou link do YouTube.*\n\n*[ 💡 ] Exemplo:* _${usedPrefix + command} Filipe Ret - Invicto_`;
    ...
    if (size >= limit_a2) {  
    await conn.sendMessage(m.chat, {text: `*[ ℹ️ ] Seu áudio:* _${audio}_`}, {quoted: m});
    return;    
    }     
    ...
    if (size2 >= limit2) {  
    await conn.sendMessage(m.chat, {text: `_*< SPY - PLAY />*_\n\n*[ ℹ️ ] Seu vídeo:* _${video}_`}, {quoted: m});
    return;    
    }     
...
async function search(query, options = {}) {
  const search = await yts.search({query, hl: 'pt', gl: 'BR', ...options});
