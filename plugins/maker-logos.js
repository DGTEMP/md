...
const handler = async (m, {conn, args: [effect], text: txt, usedPrefix, command, name}) => {
  ...
  const res = await maker(effectoSelect?.url, [...text]).catch(_ => { throw '*[❗] Falta o texto para personalizar sua logo*' })
   if (typeof res == 'number') throw res == -1 ? `*[❗𝐈𝐍𝐅𝐎❗] 𝙴𝙵𝙴𝙸𝚃𝙾 ${effect} 𝙽𝙰𝙾 𝙴𝚂𝚃𝙰 𝙽𝙰 𝙻𝙸𝚂𝚃𝙰 𝙳𝙴 𝙴𝙵𝙴𝙸𝚃𝙾𝚂*` : `*[❗𝐈𝐍𝐅𝐎❗] 𝚄𝚂𝙾 𝙲𝙾𝚁𝚁𝙴𝚃𝙾 𝙳𝙴 𝙲𝙾𝙼𝙰𝙽𝙳𝙾 ${usedPrefix + command} ${effect} ${new Array(res).fill('texto').map((v, i) => v + (i ? i + 1 : '')).join('|')}*`;
  await conn.sendMessage(m.chat, {image: {url: res.image}, caption: `*𝚃𝙾𝙼𝙰 𝚃𝚄𝙰 𝙸𝙼𝙰𝙶𝙴𝙽 𝙿𝙴𝚁𝚂𝙾𝙽𝙰𝙻𝙸𝚉𝙰𝙳𝙰!!*\n*𝙴𝙵𝙴𝙸𝚃𝙾: ${effect}*`}, {quoted: m});  
};
handler.help = ['logos'];
handler.tags = ['nulis'];
